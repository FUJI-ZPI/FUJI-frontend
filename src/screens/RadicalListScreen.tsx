...
const RadicalListScreen: React.FC<ScreenProps> = ({navigation, route}) => {
    ...
    const fetchRadicalList = async (level: number) => {
        setLoading(true);
        setError(null);
        try {
            ...
            const res = await fetch(
                `${process.env.EXPO_PUBLIC_BACKEND_URL}/api/v1/radical/level/${level}`,
                {headers}
            );
            ...
            const data: RadicalItem[] = await res.json();
            setRadicalList(data);
            ...
            console.error('Failed to fetch radical list:', e);
    ...
    const onSelectRadical = (item: RadicalItem) => {
        navigation.navigate('RadicalDetail', {
    ...
    if (loading) {
        return (
            ...
            >
                ...
                <Text style={localStyles.pageInfo}>Loading radicals...</Text>
            </SafeAreaView>
        );
    }
    ...
    if (error) {
        return (
            ...
            >
                ...
                <TouchableOpacity onPress={() => fetchRadicalList(level)}>
    ...
    const totalCount = radicalList.length;
    const startIndex = currentPage * ITEMS_PER_PAGE;
    const endItemNumber = Math.min((currentPage + 1) * ITEMS_PER_PAGE, totalCount);
    const currentRadicalPage = radicalList.slice(startIndex, startIndex + ITEMS_PER_PAGE);
                ...
                <FlatList
                            ...
                            <View style={localStyles.paginationContainer}>
                                ...
                                >
                                    <Ionicons name="chevron-back" size={20}
                                              color={(currentPage === 0 || totalPages <= 1) ? colors.textMuted : colors.text}/>
                                </TouchableOpacity>

                                <Text style={localStyles.pageInfo}>
                                    {t('Radicals {{start}}-{{end}} of {{total}}', {
                                        start: startIndex + 1,
                                        end: endItemNumber,
                                        total: totalCount
                                    })}
                                ...
                                >
                                    <Ionicons name="chevron-forward" size={20}
                                              color={(currentPage >= totalPages - 1 || totalPages <= 1) ? colors.textMuted : colors.text}/>
                                </TouchableOpacity>
                            </View>
                        </>
                    }

                    data={currentRadicalPage}
                    ...
                    ListEmptyComponent={
                        <Text style={localStyles.noItemsText}>
                            {t('No radicals found for this level.')}
                        </Text>
                    }
                />
            </View>
