...
const RelatedSubjectTile: React.FC<{ subjectId: number, onPress: () => void, title?: string }> = ({ subjectId, onPress, title }) => (
    <TouchableOpacity onPress={onPress} style={localStyles.kanjiTile}>
        <Ionicons name="apps-outline" size={28} color={primaryGreen} />
        <Text style={localStyles.kanjiTileText}>{title || `Subject #${subjectId}`}</Text>
...
const ReadingGroup: React.FC<{ title: string, readings: Reading[] }> = ({ title, readings }) => {
    ...
    return (
        <View style={localStyles.readingGroupContainer}>
            ...
            <View style={localStyles.tagContainer}>
                {readings.map(r => (
                    <View key={r.reading} style={[localStyles.tag, r.primary && localStyles.primaryTag]}>
                        <Text style={[localStyles.tagText, r.primary && localStyles.primaryTagText]}>{r.reading}</Text>
                    </View>
                ))}
            </View>
        </View>
...
const KanjiDetailScreen: React.FC<ScreenProps> = ({ navigation, route }) => {
    ...
    useEffect(() => { fetchKanjiDetails(kanjiUuid); }, [kanjiUuid]);

    const navigateToSubject = (subjectId: number) => {
        console.log(`📘 [SUBJECT] Would navigate to Subject #${subjectId}`);
        Alert.alert(
    ...
    // Znajdź pierwsze 'primary' czytanie jako główne, lub pierwsze dowolne
    const primaryReading = kanjiData.readings.find(r => r.primary)?.reading || kanjiData.readings[0]?.reading || '';
    const primaryMeaning = kanjiData.meanings.find(m => m.primary)?.meaning || '';
    ...
    const renderMeaningTab = () => (
        <Card style={localStyles.tabContentCard}>
            <View style={localStyles.sectionHeader}>
                ...
                <MnemonicTooltipButton icon="bulb-outline" mnemonicText={kanjiData.meaning_mnemonic} />
            </View>
            ...
            {kanjiData.meaning_hint && (
                <View style={localStyles.hintContainer}>
                    <Ionicons name="information-circle-outline" size={18} color={accentBlue} />
                    <Text style={localStyles.hintText}>{kanjiData.meaning_hint.replace(/<.*?>/g, '')}</Text>
                </View>
            )}
        </Card>
    );
    ...
    const renderReadingTab = () => (
        <Card style={localStyles.tabContentCard}>
            ...
            {kanjiData.reading_hint && (
                <View style={[localStyles.hintContainer, {marginTop: spacing.base}]}>
                    <Ionicons name="information-circle-outline" size={18} color={accentBlue} />
                    <Text style={localStyles.hintText}>{kanjiData.reading_hint.replace(/<.*?>/g, '')}</Text>
                </View>
            )}
        </Card>
    );
    ...
    const renderRelatedTab = () => (
        <>
            ...
            {/* Amalgamation Subjects (Used in Vocab) */}
            ...
            {kanjiData.component_subject_ids.length === 0 &&
             kanjiData.amalgamation_subject_ids.length === 0 &&
             kanjiData.visually_similar_subject_ids.length === 0 && (
                <Text style={localStyles.noItemsText}>No related items found.</Text>
            )}
        </>
    );
            ...
            <ScrollView contentContainerStyle={localStyles.scrollContent}>
                ...
                <View style={localStyles.characterBox}>
                    ...
                    <View style={localStyles.readingContainer}>
                        ...
                        <MnemonicTooltipButton icon="chatbubble-ellipses-outline" mnemonicText={kanjiData.reading_mnemonic} />
                    </View>
                </View>

                <View style={localStyles.tabContainer}>
                    <TabButton icon="book-outline" label="Meaning" isActive={activeTab === 'meaning'} onPress={() => setActiveTab('meaning')} />
                    <TabButton icon="chatbubbles-outline" label="Reading" isActive={activeTab === 'info'} onPress={() => setActiveTab('info')} />
                    <TabButton
...
const localStyles = StyleSheet.create({
    ...
    characterText: { fontSize: 90, fontWeight: '400', color: accentBlue, marginBottom: spacing.small },
    ...
    kanjiGrid: {
        flexDirection: 'row', flexWrap: 'wrap', gap: spacing.base, justifyContent: 'flex-start',
        marginTop: spacing.base
    },
